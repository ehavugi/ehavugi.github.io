# ehavugi.github.io
Personal website of Emmanuel Havugimana (PhD Student)

## Website Description
Mostly hosting demos/prototypes related to my research. You can find more information about me on my [ old website](https://www.havugimana.com) and [new website](https://ehavugi.github.io/)(this github repository, to be linked later to [havugimana.com](https://www.havugimana.com) ).

# My research 


## Demos
The `code` folder contains some demos/prototypes of my research. You can find more information about them in the `code` folder README.

### Examples of demos: 


- [Code runner](https://ehavugi.github.io/code/pythonrunner/)

- ECCE 2025 data repo : [ECCE 2025-data](https://github.com/ehavugi/ECCE-2025-data)
 
 - Planar PCB Cost Model : [Cost Model](https://ehavugi.github.io/costmodel)

- [LLM test (in browser)](https://ehavugi.github.io/code/llmtest/)


### Code forks from other researchers (Hosted demos or modified repos)

This is so that others can fork more or modify more the following repositories:

* Mag-Net Hub - Mag-Net (with ASU coreloss model added): [https://github.com/ehavugi/mag-net-hub](https://github.com/ehavugi/mag-net-hub) 
* Magnet-Engine - Magnet-Engine (with ASU coreloss model added):[https://github.com/ehavugi/magnet-engine](https://github.com/ehavugi/magnet-engine)
* Princeton MagNet - Princeton MagNet (pure-python simulation alternative to PLECS): [https://github.com/ehavugi/magnet](https://github.com/ehavugi/magnet)

* guszhang/circuitpaint fork [repo](https://github.com/ehavugi/circuitpaint) 
    +   (Extra host demo) :  [https://ehavugi.github.io/circuitpaint/](https://ehavugi.github.io/circuitpaint/))

## License

